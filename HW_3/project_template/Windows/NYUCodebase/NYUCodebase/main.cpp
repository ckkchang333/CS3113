...
GameState& setup() {
	...
	for (int i = 0; i < NUM_ENEMIES_ROW; ++i) {
		game.enemies[i * NUM_ENEMIES_ROW] = Entity(10, -1.75f + pos_x * .75f, 3.25f, 0.75f, 0.75f, 0.0005, true, 1.0f);
		game.enemies[i * NUM_ENEMIES_ROW + 1] = Entity(49, -1.75f + pos_x * .75f, 2.50f, 0.75f, 0.75f, 0.0005, true, 1.0f);
		game.enemies[i * NUM_ENEMIES_ROW + 2] = Entity(52, -1.75f + pos_x * .75f, 1.75f, 0.75f, 0.75f, 0.0005, true, 1.0f);
		game.enemies[i * NUM_ENEMIES_ROW + 3] = Entity(55, -1.75f + pos_x * .75f, 1.00f, 0.75f, 0.75f, 0.0005, true, 1.0f);
		game.enemies[i * NUM_ENEMIES_ROW + 4] = Entity(58, -1.75f + pos_x * .75f, 0.25f, 0.75f, 0.75f, 0.0005, true, 1.0f);
		++pos_x;
	}
...
void UpdateGame(GameState& game, float elapsed) {
	...
	for (int i = 0; i < NUM_ENEMIES_ROW; ++i) {
		bool turn = false;
		if (game.enemies[i * NUM_ENEMIES_ROW].x < -2.1f || (game.enemies[i * NUM_ENEMIES_ROW + 4].x + game.enemies[i * NUM_ENEMIES_ROW + 4].width) > 2.4f) {
			turn = true;
		}
		for (int j = 0; j < NUM_ENEMIES_ROW; ++j) {
			if (turn)
				game.enemies[i * NUM_ENEMIES_ROW + j].x_direction *= -1.0f;
			game.enemies[i * NUM_ENEMIES_ROW + j].x += game.enemies[i * NUM_ENEMIES_ROW + j].velocity * game.enemies[i * NUM_ENEMIES_ROW + j].x_direction;
